...
| Type | 워크플로우 | 특징 |
|------|-----------|------|
| feature | Todo → Planning → In Progress → In Review → Done | prd.md + plan.md 생성, Micro-tasking 구현, auto-verify |
| improvement | size별 분기 — light: In Progress 직행 / standard: Planning → In Progress | light(bug 수준) / standard(Planning 경유) |
| bug | Todo → In Progress → In Review → Done | Git 폴더 + note.md 생성, Root Cause 분석 |
