...
3. [.claude/templates/setup.md](.claude/templates/setup.md)의 설정 가이드를 따라 Linear 워크스페이스 + MCP 설정 진행
...
| Skills | 14 | dev-pipeline, gen-hub, gen-plan, implement, bug-fix, improvement-fix, cleanup, issue-close, triage, feedback, investigation, verify, test, spec |
| Commands | 12 | /등록, /활성화, /점검, /커밋, /피드백, /조사, /검증, /병합, /현황, /스펙, /정리, /릴리스 |
...
| Type | 워크플로우 | 특징 |
|------|-----------|------|
| feature | Todo → Planning → In Progress → In Review → Done | spec.md + plan.md 생성, ralph 루프 구현, verify (auto/manual) |
| improvement | size별 분기 — light: In Progress 직행 / standard: Planning → In Progress | light(bug 수준) / standard(Planning 경유) |
| bug | Todo → In Progress → In Review → Done | Git 폴더 + spec.md 생성, deep-dive Root Cause 분석 |
