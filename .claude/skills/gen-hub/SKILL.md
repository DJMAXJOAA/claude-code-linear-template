...
`/등록` 커맨드에서 호출되어, Linear Issue를 생성하고 Git에 `docs/issue/{LINEAR-ID}/` 폴더를 생성한다. **feature, improvement만** 폴더를 생성한다. bug는 Git 폴더를 생성하지 않는다.
...
| 단계 | 행위 |
|------|------|
| 0a | **Spec 로드**: `--from-spec` 경로의 `overview.md` + `requirements.md` 로드. FR 테이블에서 FR-ID 목록 수집 |
...
| 5 (G3) | **Git 폴더 생성**: `docs/issue/{LINEAR-ID}/` 디렉토리 생성 (feature, improvement만. bug는 생성하지 않음) |
...
| Git 폴더 | `docs/issue/{LINEAR-ID}/` (feature, improvement만 생성. bug는 미생성). Documents는 파이프라인 진행 시 동적 추가 |
