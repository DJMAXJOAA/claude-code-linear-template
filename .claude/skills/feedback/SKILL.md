...
| 4 (G3) | **저장**: 승인된 내용을 대상 문서에 기록. backlog 유형은 gen-hub Skill을 호출하여 새 Issue + note.md 생성 |
| 5 (G3) | 파이프라인 진행 중이면 `note.md > ## Work Log`에 Feedback 기록 + Linear comment에도 기록 |

## Output

| 항목 | 내용 |
|------|------|
| 대상 문서 | 피드백이 등록된 문서 (rules, guides, note.md 등) |
| note.md | Work Log에 Feedback 기록 (파이프라인 진행 중일 때) |
...
| `limitation` | `note.md > ## Work Log` (해당 Issue 폴더) + Linear comment | Issue 진행 중 전용 — Known Limitations |
...
| Known Limitations | `docs/issue/{LINEAR-ID}/note.md > ## Work Log` + Linear comment | ✅ 프로젝트 | 현재 구현의 제약, 임시 구현 |
