...
| 트리거 | 행동 |
|--------|------|
| Plan+CL 생성 시 | CL S1 태스크별 sub-issue 생성 (best-effort. 실패 시 진행 중단 안 함) |
| 태스크 시작/완료 | sub-issue state → In Progress / Done |
| CL S1 태스크 변경 | 추가: 새 sub-issue 생성. 삭제/변경: 수동 정리 |
| 동기화 방향 | CL S1 → Linear (단방향). CL이 SSOT |
...
| 단계 | 행위 |
|------|------|
| 3-1 | **태스크 선택**: CL S1 의존성 기반 — Dependencies 모두 `done`인 태스크 중 ID 순 |
| 3-2 | **코드 구현**: 모드에 따라 직접 작성(a) 또는 executor 위임(b) |
| 3-3 | **빌드 확인**: 린트 + 타입체크 + 테스트 통과 |
| 3-4 | **CL S1 갱신**: 태스크 상태 → `done` |
...
| **(c) ralph** | `oh-my-claudecode:ralph`로 CL S1 전체 자동 | ralph 자체 판단 |
...
plan.md + cl.md 기반으로 유저가 직접 구현하고, AI는 검증만 수행한다.

| 단계 | 행위 |
|------|------|
| d-1 | CL S1 태스크 목록 요약 출력 (유저 참조용) |
| d-2 | `AskUserQuestion`: "구현을 완료하면 알려주세요" — 유저 작업 대기 |
| d-3 | 유저 완료 신호 수신 |
| d-4 | git diff 분석 → CL S1 태스크와 변경 내용 대조 |
| d-5 | CL S1 태스크 상태 일괄 갱신 (완료된 태스크 체크) |
...
> d-4에서 CL S1과 diff가 불일치하면 `AskUserQuestion`으로 확인.
