...
| 2 (G4) | **ralph 루프 실행**: `oh-my-claudecode:ralph`로 plan.md Tasks 전체 자동 실행 (인터뷰 없이 즉시 시작) |
...
ralph 루프 고정. 인터뷰 없이 즉시 `oh-my-claudecode:ralph`로 plan.md Tasks 전체를 자동 실행한다. deslop pass 내장 (v4.9.0+). ralph가 plan.md Tasks 기반 완료까지 반복한다.
...
> ralph가 plan.md Tasks 전체를 자동 실행하며, 태스크별 실행·검증·갱신을 자체 판단으로 수행한다.
...
| 단계 | 행위 |
|------|------|
| 4-1 | plan.md Tasks 모든 태스크 상태 = `done` 확인. **단, 블로킹 리셋된 태스크가 존재하면 pipeline.md §4-4a 대기 상태로 전환 (verify 미호출)** |
