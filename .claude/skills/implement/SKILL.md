...
| 4-3 | **verify 호출**: verify 스킬 호출하여 SC + plan.md Verification 검증 수행. `verify_mode` 파라미터를 그대로 전달 (auto: 자동 실행, manual: 검증 계획 출력 후 사용자 확인) |
| 4-4 | verify **PASS** 시: 미커밋 변경사항 Git 커밋 + §In Review 출력 양식 실행 |
| 4-4a | verify **SKIP** 시 (Manual Mode 스킵 선택): PASS와 동일하게 In Review 전이. `pending_manual_review`에 '사용자 스킵 — 전체 검증 생략' 포함. §In Review 출력 양식의 `verify_result`를 `SKIP`으로 설정 |
...
- [ ] 빌드: {PASS/FAIL/SKIP}
- [ ] 린트: {PASS/FAIL/SKIP}
- [ ] 테스트: {PASS/FAIL/SKIP} ({통과}/{전체})
- [ ] verify SC: {PASS/FAIL/SKIP}
...
- payload:
  ...
  - verify_result: {verify 결과 — PASS/FAIL/SKIP + SC 통과/전체, Verification 통과/전체. SKIP 시 현황 생략}
