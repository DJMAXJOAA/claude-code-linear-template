...
|------|-----------|-----------|-------------|-----------|
| feature | Light | Todo→Planning→In Progress→In Review→Done | explore → plan(omc) → executor → verify | spec.md(간소) + plan.md(간소) |
...
| Todo | **Linear → Planning 즉시 전이** → **explore** → spec.md(간소) 산출 → plan(omc) + gen-plan(Light) | Planning |
...
| 단계 | OMC 도구/스킬 | 역할 |
|------|-------------|------|
| Pre-Plan (feature Light) | explore → plan(omc) + gen-plan(Light) | 탐색 → 간소 계획 산출 |
