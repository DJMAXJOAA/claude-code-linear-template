...
| feature | Deep | Todo→Planning→In Progress→In Review→Done | explore → deep-interview(skill) → ralplan(--deliberate) → autopilot → verify | spec.md(심층) + plan.md + technical.md |
| improvement | Light | Todo→In Progress→In Review→Done | code-reviewer → plan(터미널) → executor → verify → simplify(skill) | Git 문서 없음 |
| improvement | Standard | Todo→Planning→In Progress→In Review→Done | code-reviewer → plan → architect → executor → verify → simplify(skill) | spec.md + plan.md + technical.md |
| improvement | Deep | Todo→Planning→In Progress→In Review→Done | code-reviewer + security-reviewer → deep-interview(skill) → ralplan(--deliberate) → autopilot → verify | spec.md + plan.md + technical.md |
...
| Planning (plan.md 존재) | plan.md 확인 → **executor 단독 실행** (Post-Plan Q/A 스킵 — dev-pipeline §Post-Plan Q/A 참조) | In Progress |
| In Progress | executor 단독 실행 — plan.md Tasks 진행 | In Progress (태스크 진행) |
| In Progress (모든 태스크 done) | verify 호출 (auto/manual) → PASS 시 In Review 전이 | In Review |
...
|-----------|-----------|-------------|
| (전 State 공통) | **Sub-issue 상태 확인** → 미완료 시 리마인딩 + 사용자 선택 (§Sub-issue 리마인딩) | — |
| Todo | **Linear → Planning 즉시 전이** → **explore** → **deep-interview(skill)** → spec.md(심층) 산출 → ralplan(gen-plan --deliberate) | Planning |
...
| In Progress | executor → verify → simplify(skill) — progress.txt 갱신 (improvement-fix 담당) | In Progress (태스크 진행) |
...
|-----------|-----------|-------------|
| (전 State 공통) | **Sub-issue 상태 확인** → 미완료 시 리마인딩 + 사용자 선택 (§Sub-issue 리마인딩) | — |
| Todo | **intensity 판별** → **code-reviewer + security-reviewer** (병렬) → deep-interview(skill) → ralplan(gen-plan --deliberate) | Planning |
| Planning (plan.md 존재) | **Post-Plan Q/A** → autopilot 실행 | In Progress |
| In Progress | autopilot — prd.json+progress.txt 갱신 | In Progress (태스크 진행) |
| In Progress (점검 P1) | dev-pipeline: plan.md 수정 조율 → autopilot 재호출 | In Progress (태스크 재진행) |
| In Progress (모든 태스크 done) | verify 호출 (auto/manual) → PASS 시 In Review 전이 | In Review |
...
| Pre-Plan (feature/improvement Deep) | explore → deep-interview(skill) → ralplan(--deliberate) | 탐색 → 심층 인터뷰 → 신중 계획 산출 |
| Pre-Plan (improvement Light/Standard) | code-reviewer → plan/architect | 코드 리뷰 → 계획 |
| Pre-Plan (improvement Deep) | code-reviewer + security-reviewer → deep-interview(skill) | 코드+보안 리뷰(병렬) → 심층 인터뷰 |
...
| 코드 품질 (improvement-Light/Standard) | simplify(skill) | 구현 후 코드 품질 검토 및 개선. Deep은 autopilot 내부에서 처리 |
