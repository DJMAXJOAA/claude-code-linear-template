---
name: dev-pipeline
description: "Linear Issue 상태(State)와 type(Label)을 조회하고, note.md Handoff + prd.md + technical.md 기반 컨텍스트를 로드하여 적합한 스킬로 라우팅하는 통합 파이프라인 라우터."
---
...
| 2 (G1) | **Git 문서 확인**: `docs/issue/{LINEAR-ID}/` 폴더 존재 여부 확인. **폴더 미존재 시** `docs/issue/{LINEAR-ID}/` + note.md 자동 생성 (gen-hub 템플릿 참조) + Linear description Documents 갱신. 존재 시 note.md Handoff 섹션에서 컨텍스트 로드 |
...
| 5a (G1) | **Git 필독 규칙**: `/활성화` 시 — Linear → note.md Handoff → prd.md → technical.md(있으면) 순서로 컨텍스트 로드. 재개 시 — note.md Handoff → plan.md Tasks → technical.md(있으면) 순서로 로드 |
...
1a. note.md Work Log에 P1 수정 기록 추가: 'P1 계획수정: {수정 사유} — Tasks {리셋 ID 목록}'
...
| 1a | **관련 문서 환류**: related issue의 `note.md`(Checkpoints) + `plan.md` 읽기 (최대 5개). 요약하여 step 2에서 제시. 구현 결과 섹션은 gen-plan에서 별도 참조 |
| 2 | **스코프/조사 인터뷰**: step 1a 환류 결과를 요약 제시한 뒤, `AskUserQuestion`으로 (a) explore 탐색 범위(전체/특정 모듈/최소) (b) `/조사`(investigation) 실행 여부 (c) 복잡한 인과 분석 필요 시 `deep-dive`(trace→deep-interview) 실행 여부를 확인 |
...
| 5 | **prd.md 인터뷰 반영**: Phase 1 조사 결과 + Phase 2 설계 결정을 gen-plan의 prd/technical 생성 인풋으로 전달. gen-plan 호출 전 수집 정보를 note.md Work Log에 선저장 |
...
Planning 상태에서 prd.md + plan.md 존재 확인 후, 다음 행동을 사용자에게 선택받는 인터랙션.

| 단계 | 행위 |
|------|------|
| 1 | Plan 핵심 요약 제시 (prd.md + plan.md Tasks 기반) |
| 2 | `AskUserQuestion`으로 사용자 선택: **(a)** 바로 구현 **(b)** Plan Q&A 인터뷰 **(c)** 합의 리뷰 (Architect + Critic) |
| 3a | 바로 구현: prd.md + plan.md가 미커밋 상태이면 Git 커밋 수행 → Linear State → In Progress → implement Skill 호출 |
| 3b | Q&A 인터뷰: Plan 내용 질의응답 → 완료 후 다시 선택(2단계) |
| 3c | 합의 리뷰: 아래 §합의 리뷰 프로세스 수행 → 결과 제시 후 다시 선택(2단계) |

> **AI 권장 분기**: plan.md Tasks 4개 이상인 feature → (c)에 `(AI 권장)`. 그 외(태스크 3개 이하 또는 improvement-standard) → (a)에 `(AI 권장)`.
...
| 단계 | 행위 |
|------|------|
| R1 | **Architect + Critic 병렬 리뷰**: 두 에이전트를 동시에 호출하여 독립 리뷰를 수행한다. 각 에이전트는 prd.md + plan.md + Linear description을 입력으로 받는다 |
...
| 3 | Linear comment + note.md에서 블로킹 여부 교차 검증 |
| 4 | 리마인딩 테이블 출력 (아래 형식) |
