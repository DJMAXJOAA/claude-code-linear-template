...
| 단계 | 행위 |
|------|------|
| 1 | **Linear 상태 조회**: Linear MCP로 Issue의 현재 State + type(Label) + assignee 조회 |
| 2 | **Git 문서 확인**: `docs/issue/{LINEAR-ID}/_index.md` 존재 여부 + Documents 테이블에서 기존 문서 확인 |
| 3 | **type 판별**: Label에서 type 추출 (`feature` / `bug` / `improvement` / `research`) |
| 4 | **라우팅 테이블 참조**: 현재 State + type 조합으로 다음 스킬 결정 |
| 5 | **상태 전이**: 다음 단계 진입 시 Linear MCP로 State 갱신 |
| 6 | **스킬 호출**: 결정된 스킬을 호출하며, Linear ID + type + 관련 컨텍스트 전달 |
...
| 상태 갱신 | Linear Issue State를 다음 단계로 전이 |
...
| Verifying | verify Skill → feature-close | Done |
| Done | 완료 안내 | — |
...
| Verifying | verify Skill → close 처리 | Done |
| Done | 완료 안내 | — |
...
| 단계 | 행위 |
|------|------|
| 1 | Root Cause 분석 — 코드베이스 탐색 + `notes.md`에 기록 (lazy-creation) |
...
| 항목 | 현행 | 신규 |
|------|------|------|
| 상태 소스 | Hub frontmatter `status` | Linear Issue State (MCP 조회) |
| 분기 기준 | intensity (light/standard) | type (feature/bug/improvement/research) |
| Issue 처리 | issue-pipeline 별도 스킬 | dev-pipeline에 통합 (type: bug) |
| backlogs 갱신 | `backlogs/features.md` 상태 행 갱신 | 제거 — Linear가 상태 SSOT |
...
| 시점 | MCP 도구 | 용도 |
|------|---------|------|
| 상태 조회 | `linear_search_issues` | Issue의 현재 State + type(Label) + assignee 조회 |
| 상태 전이 | `linear_update_issue` | State 변경 (Backlog→Planning, Planning→In Progress 등) |
| 관련 Issue 조회 | `linear_search_issues` (Label 필터) | Pre-Plan Q/A 컨텍스트 수집 |
