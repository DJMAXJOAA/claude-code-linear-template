...
| 6 (G4) | **스킬 호출**: 결정된 스킬을 호출하며, Linear ID + type + intensity + verify_mode + **`linear_payload`** + 관련 컨텍스트 전달 |
...
### Step 5: Verify Mode 선택

`AskUserQuestion` 1회로 verify mode를 선택한다. 구현 완료 후 검증 방식을 결정한다.

| 모드 | 설명 |
|------|------|
| **Auto** `(AI 권장)` | 구현 완료 후 verify 스킬이 자동으로 검증 실행. 사용자 개입 없이 PASS/FAIL 판정 |
| **Manual** | 구현 완료 후 검증 계획(Phase별 검증항목·방법·기대결과)을 터미널에 출력. 사용자가 확인/수정 후 검증 실행 |

> 선택된 verify_mode를 하류 스킬에 전달한다. Label로 저장하지 않는다 (세션 내 파라미터).

### Step 6: type별 스킬 라우팅

intensity + verify_mode 인자를 포함하여 type별 스킬로 라우팅:
- `feature` → intensity에 따라 dev-pipeline 내부 분기 (Pre-Plan Q/A 등)
- `improvement` → improvement-fix 스킬 호출 (intensity + verify_mode 인자 전달)
- `bug` → bug-fix 스킬 호출 (intensity + verify_mode 인자 전달)
