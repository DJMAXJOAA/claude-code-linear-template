...
| 3a | **탐색 결과 출력**: step 3 조사 결과 + step 1a 환류 결과를 종합하여 요약 제시. 발견된 아키텍처 제약, 관련 코드 패턴, 주의점 등을 정리 |
| 4 | **설계 인터뷰**: `AskUserQuestion`으로 설계 결정 위주 인터뷰. 탐색 결과에서 도출된 (a) 애매한 부분 — 설계 선택지 + AI 권장안 제시 (b) 보완이 필요한 부분 — 구체화 필요 항목 질문 (c) 중요 설계 결정안 재확인 — 권장안과 함께 제시. 스펙 연결, Decisions, 리스크, 범위를 이 단계에서 확정 |
| 5 | **SC 확정**: 모든 설계 결정이 완료된 후, `AskUserQuestion`으로 최종 SC 확정 권장안 제시. 기존 SC(Linear description)를 설계 결정 결과에 맞게 조정한 버전을 AI가 제안하고 사용자가 확인/수정 |
| 6 | **SC를 Linear description에 기록**: 확정된 SC를 Linear Issue description의 `## Success Criteria` 섹션에 삽입 |
| 7 | **_index.md 갱신**: `## Decisions`에 설계 결정 기록, `## Notes`에 조사 결과 요약 + 스코프 범위 기록. gen-plan 호출 전 수집 정보를 Git에 선저장 |
