...
| _index.md | `docs/issue/{LINEAR-ID}/_index.md` — 기존 문서 확인 (**feature/improvement-standard만**. bug/improvement-light는 없음) |
| plan.md | `docs/issue/{LINEAR-ID}/plan.md` — 설계 결정 비교 대상 (**feature/improvement-standard만**) |
| cl.md | `docs/issue/{LINEAR-ID}/cl.md` — 태스크 완료 상태 확인 (**feature/improvement-standard만**) |
| 검증 결과 | verify Skill 산출물 |
| 참조 문서 목록 | Documents 테이블(_index.md) 또는 Linear description Documents 섹션에서 추출한 참조 경로 목록 |

## Process

### type별 경로 분기

| type | 실행 단계 | _index.md | description 미러링 | 후행 환류 | spec 연동 |
|------|----------|-----------|-------------------|----------|----------|
| **feature** | 1→2→3→4→5→5a→6→7→8 (전체) | ✅ 갱신 | ✅ | ✅ | ✅ |
| **improvement-standard** | 1→2→3→4→5→5a→6→7→8 | ✅ 갱신 | ✅ | ✅ | ✅ (Spec 참조 존재 시) |
| **improvement-light** | 1→2→3→4→5→8 | ❌ 없음 | ❌ | ❌ | ❌ |
| **bug** | 1→2→3→4→5→8 | ❌ 없음 | ❌ | ❌ | ❌ |
...
| 3 (G3) | **_index.md 구현 결과 기록**: _index.md가 존재하는 type만 수행. 아래 §_index.md 기록 참조 |
...
| 항목 | 적용 type | 내용 |
|------|----------|------|
| _index.md | feature, improvement-standard | `## 구현 결과` 섹션 생성/갱신 |
...
| 후행 Issue | feature, improvement-standard | _index.md Notes 환류 메시지 + Linear comment (대상 존재 시) |
| spec 문서 | feature, improvement-standard | Related Issues + Change Log 갱신 (링크된 spec 존재 시) |
...
> **bug/improvement-light**: _index.md·plan.md가 없으므로 Linear comment(수정 결과 요약)과 `git log`(관련 커밋)에서 구현 결과를 수집한다.
...
| 조건 | 행동 |
|------|------|
| _index.md 존재 (feature, improvement-standard) | `## 구현 결과` 섹션 lazy-create. 아래 §구현 결과 섹션 템플릿으로 생성. 이미 존재하면 갱신 |
| _index.md 미존재 (bug, improvement-light) | **스킵**. Git 문서를 생성하지 않는다. 구현 결과는 Linear comment에만 기록 |
...
issue-close 시 _index.md의 최종 처리 내용을 Linear Issue description에 1회성 스냅샷으로 반영한다.

> **적용 대상**: feature, improvement-standard만. bug/improvement-light는 _index.md가 없으므로 스킵.
...
> **적용 대상**: feature, improvement-standard. _index.md Documents 테이블에 Spec 행이 존재하는 경우에만 실행. bug/improvement-light는 _index.md가 없으므로 스킵.
...
| 7-4 | spec `_index.md`의 `## Change Log`에 변경 이력 추가. 확장된 4-컬럼 형식(날짜/FR-ID/변경유형/변경내용) 사용, 영향 FR-ID 특정 (spec-template.md 참조) |
| 7-5 | spec frontmatter `updated` 날짜 갱신 |
| 7-6 | `docs/spec/_index.md` 글로벌 목록 테이블 자동 갱신 |

> spec 본문 갱신(`requirements.md`, `technical.md` 등)은 §8 참조 문서 내용 동기화에서 통합 처리. §7은 메타데이터(Related Issues, Change Log, frontmatter) 전용.
...
| type | 수집 소스 | 비고 |
|------|----------|------|
| feature | `_index.md` Documents 테이블 + `plan.md` §3a Traceability | hub + FR→Task 매핑 |
| improvement-standard | `_index.md` Documents 테이블 | hub 기반 |
...
| Index 행 (`_index.md`), Plan, Checklist | 스킵 (이미 다른 단계에서 처리) |
