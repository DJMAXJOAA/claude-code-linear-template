...
| 선택지 | 설명 | 후속 |
|--------|------|------|
| **(a)** 새 이슈 문서 생성 | Linear 이슈 + note.md + prd.md(인터뷰) 생성 | 경로 A |
| **(b)** 기존 이슈에 반영 | 기존 note.md Work Log 갱신 + Linear comment | 경로 B |
...
| A-5 | note.md 생성: gen-hub 템플릿 참조. **전 type(feature/improvement/bug) 모두 Git 폴더 + note.md 생성** |
| A-6 | `AskUserQuestion`: prd.md 생성 여부 인터뷰. 승인 시 diff 추론 결과 기반 prd.md 생성 (사후 기록이므로 간소화) |
| A-7 | 구현 결과를 note.md Work Log에 즉시 기록: issue-close 템플릿([skills/issue-close/templates/implementation-result.md](../issue-close/templates/implementation-result.md)) 참조 |
...
| B-2 | `docs/issue/{LINEAR-ID}/note.md` 존재 확인 |
| B-3a | note.md 존재: `## Work Log`에 변경 기록 추가 |
| B-3b | note.md 미존재: Git 폴더 + note.md 생성 후 Work Log 기록. Linear comment에도 변경 요약 기록 |
...
| note.md | 생성 (전 type) | 갱신 | — | — |
| prd.md | 인터뷰 후 생성 (선택) | — | — | — |
| 구현 결과 | note.md Work Log 기록 | 갱신 | — | — |
...
|------|------|------|------------|----------|
| note.md | 전체 (경로 A 신규 생성 시) | 생성 | gen-hub | 사후 문서화는 파이프라인 외부 흐름. gen-hub의 Todo 생성과 cleanup의 Done 생성은 프로세스가 상이 |
| note.md | `## Work Log` | 생성/갱신 | issue-close | 사후 문서화 시 issue-close 미경유. cleanup이 동일 템플릿을 참조하여 직접 기록 |
| prd.md | 전체 (경로 A 인터뷰 승인 시) | 생성 | gen-plan | 사후 문서화 시 gen-plan 미경유. diff 추론 기반 간소화 생성 |
