...
| A-6 | **spec.md 생성**: diff 추론 결과 기반으로 OMC 양식 간소화 버전 작성 (Goal + Constraints + 작업 내용). `.claude/templates/git-doc-formats.md`를 참조하여 frontmatter 래핑 (`type: spec`, `issue_type`, `linear_id`, `title`, `created`) |
| A-7 | 구현 결과를 **Linear comment**에 기록: `linear-comment-writer` 에이전트를 호출하여 완료 코멘트를 작성하라. Input: linear_id={LINEAR-ID}, comment_type=completion, issue_type={type}, payload={구현 결과 요약}. 에이전트가 `.claude/templates/comment-formats.md`의 completion 형식 적용 |
| A-8 | `linear-comment-writer` 에이전트를 호출하여 Done 코멘트를 작성하라. Input: comment_type=completion (feature/improvement) 또는 completion-light (bug), issue_type={type}, payload={type별 내용} |
...
| B-5 | `linear-comment-writer` 에이전트를 호출하여 변경 요약 코멘트를 작성하라. Input: linear_id={LINEAR-ID}, comment_type=completion-light, issue_type={type}, payload={변경 요약} |
...
| Agent | 역할 | 호출 시점 |
|-------|------|----------|
| `linear-comment-writer` | 완료/변경 comment 작성 | 경로 A-7, A-8, B-5 |
