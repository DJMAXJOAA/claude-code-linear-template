...
> 후처리 에이전트(planner, executor, comment-writer)는 [references/post-pipeline.md](references/post-pipeline.md) §OMC 에이전트 연동 참조.
...
planner agent를 격리 세션에서 호출하여 triage-plan 문서를 생성하고, 사용자 검토 후 Phase별 실행. 상세: [references/post-pipeline.md](references/post-pipeline.md)
