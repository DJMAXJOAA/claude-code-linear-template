...
| in-scope × defect (In Review 사용자 점검) | **rework** | plan.md Tasks에 정리 태스크 추가 → implement 재호출로 재구현 |
...
| rework | implement (재구현) | plan.md Tasks에 정리 태스크 추가 → Linear State → In Progress → implement 재호출. verify 재실행 후 다시 In Review |
