...
| L3 | `/등록` (type: bug) | 별도 Issue 생성. **plan scope인 경우**: parent의 sub-issue로 연결 + 블로킹 라이프사이클 적용 (pipeline.md §4 참조) |
| directive | feedback Skill | directive 유형으로 전달 |
| limitation | _index.md Notes 직접 기록 | Known Limitations에 추가 |
| backlog | `/등록` | 새 Issue 생성. **plan scope인 경우**: 현재 이슈는 정상 진행하며 개선사항만 별도 등록 |
...
| P1 결과 기록 | `save_comment` | plan 수정 사유 + 변경 요약 + P1 자격 판정 근거 |
| plan-L3 sub-issue | `save_issue` (parentId 지정) | parent의 sub-issue로 생성 + `save_comment`로 블로킹 사유 기록 |
| plan-backlog | `save_issue` (id 미지정) | backlog improvement 이슈 생성 + `save_comment`로 참조 링크 기록 |
