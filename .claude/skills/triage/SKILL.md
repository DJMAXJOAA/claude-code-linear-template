...
| **plan** | 계획(prd.md/plan.md) 자체의 오류 — 구현·테스트는 plan대로 정상이나 사용자 의도(SC/Issue 설명)와 불일치 |
...
| in-scope × knowledge | **limitation** | note.md Known Limitations에 기록 |
| * × * (임시 메모) | **reminder** | note.md에 기록 |
...
| in-scope × defect (In Review 사용자 점검) | **rework** | plan.md Tasks에 정리 태스크 추가 → implement 재호출로 재구현. 조건: `/점검` 경유 + triage G2 승인 필수 (pipeline.md §1-4 예외 3) |
...
| limitation | note.md 직접 기록 | Known Limitations에 추가 |
...
| 블로킹 설정 | plan scope L3 → `note.md`에 `### Blocking: {ID}` 기록 + Linear comment 기록 (pipeline.md §4-4a) |
...
| G5 (Git 폴더 생성) | **필수 경유** | `docs/issue/{LINEAR-ID}/` 생성 |
| G6 (note.md 초기 생성) | **필수 경유** | note-template 기반 생성 |
| G7 (Linear description에 Git 경로 삽입) | **필수 경유** | Documents 섹션에 note.md 경로 기록 |
...
| 변경 내용 | type | 예시 |
|-----------|------|------|
| 문서만 변경 (note.md, plan.md, sub-issue 폴더, directive 등) | `docs` | `docs({ID}): triage 분류 결과 반영` |
