...
| verify_mode | `auto` / `manual` — dev-pipeline에서 선택하여 전달. 미전달 시 `auto` 기본값 |

## Process

verify_mode에 따라 프로세스가 분기된다.
...
| 단계 | 행위 |
|------|------|
| 1 (G1) | **Linear SC 조회**: Auto Mode 단계 1과 동일 |
| 2 (G1) | **plan.md Verification 읽기**: Auto Mode 단계 2와 동일 |
| 3 (G1) | **검증 계획 생성**: SC + Verification + 빌드/코드 품질 항목을 Phase별로 구조화하여 §검증 계획 출력 양식으로 터미널에 출력 |
| 4 (G2) | **사용자 확인**: `AskUserQuestion`으로 검증 계획 확인 요청. 선택지: (a) 수락 — 계획대로 검증 실행 `(AI 권장)` (b) 수정 — 사용자가 항목 추가/제거/변경 후 재확인 (c) 스킵 — 검증 없이 In Review로 직행 (pending_manual_review에 '사용자 스킵' 기록) |
| 5 (G4) | **검증 실행**: 확인된 검증 계획에 따라 항목별 검증 수행 |
| 6 (G4) | **검증 결과 정리**: PASS/FAIL 판정 + 근거 |
...
Manual Mode 단계 3에서 터미널에 출력하는 양식:
...
| # | 검증 항목 | 방법 | 기대 결과 |
|---|----------|------|----------|
| 1 | 린트 통과 | lint 명령 실행 | 에러 0건 |
| 2 | 타입체크 통과 | tsc --noEmit | 에러 0건 |
| 3 | 테스트 통과 | test 명령 실행 | 전체 PASS |

### Phase 2: SC 검증
| # | Success Criteria | 검증 방법 | 기대 결과 |
|---|-----------------|----------|----------|
| 1 | {SC 항목 1} | {구체적 검증 방법} | {기대 결과} |
| ... | ... | ... | ... |

### Phase 3: Verification 검증 (plan.md 존재 시)
| # | 검증 항목 | 검증 방법 | 기대 결과 |
|---|----------|----------|----------|
| 1 | {Verification 항목 1} | {구체적 검증 방법} | {기대 결과} |
| ... | ... | ... | ... |

### Phase 4: 코드 품질
| # | 검증 항목 | 검증 방법 | 기대 결과 |
|---|----------|----------|----------|
| 1 | coding rules 준수 | 코드 리뷰 | 위반 0건 |
| 2 | 범위 외 변경 없음 | git diff 확인 | Plan 범위 내 |
```

> Phase 3은 plan.md Verification 섹션이 존재하는 경우에만 출력한다 (improvement-Light, bug는 Phase 3 생략).
