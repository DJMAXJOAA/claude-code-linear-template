...
| verify_result | O | `PASS/FAIL/SKIP` |
...
| verify_result | O | `PASS/FAIL/SKIP` + SC 통과 현황 (SKIP 시 현황 생략) |
...
| 필드 | 필수 | 설명 |
|------|:----:|------|
| summary | O | 핵심 변경 1줄 요약 |
| changed_files | O | 변경 파일 수 |
| verify_result | O | verify 결과 (`PASS/FAIL/SKIP`). PASS/FAIL 시 SC 통과/전체 + Verification 통과/전체 포함. SKIP 시 "SKIP — 사용자 요청으로 검증 생략" |
| manual_test_count | O | 수동 테스트 항목 수 |

**spec-completion:**
| 필드 | 필수 | 설명 |
|------|:----:|------|
| summary | O | Spec 1줄 요약 |
| spec_path | O | spec.md 파일 경로 (예: `docs/issue/{ID}/spec.md`) |

**limitation:**
| 필드 | 필수 | 설명 |
|------|:----:|------|
| content | O | limitation 내용 |
| append_progress | O | progress.txt append 여부 (존재 시 true) |
...
2. **포함 항목 결정**: `comment_type` + `issue_type` + `intensity` 조합으로 포함 항목 결정:
   ...
   - `implement-in-review`: summary, changed_files, verify_result, manual_test_count
   - `spec-completion`: summary, spec_path
   - `limitation`: content, append_progress (true일 경우 progress.txt에도 append)
...
| `implement-in-review` | comment-formats.md > implement-in-review |
| `spec-completion` | comment-formats.md > spec-completion |
| `limitation` | comment-formats.md > limitation |
