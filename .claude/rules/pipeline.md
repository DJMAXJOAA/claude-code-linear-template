...
| 단계 | 행위 |
|------|------|
| 1 | `git log --oneline`에서 Linear ID 또는 관련 키워드 포함 커밋 필터링 |
| 2 | 커밋 0건이면 현재 브랜치의 파이프라인 진행 기간 내 최근 커밋 수집 |
| 3 | 형식: `### Git Log` + `- \`{hash}\` {message}` (최대 10건) |
| 4 | 관련 커밋 0건: `### Git Log` + "(관련 커밋 없음)" |
...
> 상세: [context.md](../../.claude/templates/context.md) §2 참조
...
| 항목 | 상세 문서 |
|------|---------|
| Pre-Compaction / 50% 규칙 | [context.md](../../.claude/templates/context.md) §3 |
...
- **프레임워크 에이전트**: `.claude/agents/`에 정의. 스킬이 on-demand로 호출. 현재: `linear-comment-writer` (Linear comment 작성 전담 — 9개 comment_type 지원). 모든 스킬의 `## OMC 에이전트 연동` 하위에 `### 프레임워크 에이전트` 서브섹션으로 호출 여부 명시
- **공유 템플릿**: `.claude/templates/`에 정의. 에이전트/스킬이 SSOT로 참조. 현재: `comment-formats.md` (9개 comment_type), `git-doc-formats.md` (4개 doc_type + progress.txt)
...
- 에이전트 모델 기본값: intensity별 라우팅
  ...
  - Deep: opus(deep-interview(skill)/ralplan/autopilot)
- 에이전트/스킬/템플릿 작성 가이드: [agent-writing-guide.md](../templates/agent-writing-guide.md), [skill-writing-guide.md](../templates/skill-writing-guide.md)
...
| 스킬 | 설명 |
|------|------|
| `/스펙` (spec) | SDD Spec 문서 생성. 5+1 게이트 파이프라인(G1~G5 + G3a), explore + external-context → deep-interview(skill) → ralplan 스킬 체인. `docs/spec/{name}/` 단위 출력 (`overview.md` + `requirements.md` + `technical.md` + `roadmap.md`(선택)). Issue 파이프라인과 독립 |
...
triage 분류(G0~G3) 완료 후, 후처리 모드를 선택하여 분류 결과를 통합 수정 계획으로 처리하는 파이프라인.

### 10-1. 모드 선택

| 모드 | 설명 | 조건 |
|------|------|------|
| Mode 1 (인터뷰) | planner agent가 주제별 인터뷰 후 triage-plan 작성 | Linear ID 필수 |
| Mode 2 (자동 계획) | planner agent가 자동으로 triage-plan 작성 → 사용자 검토 | Linear ID 필수 |
| Mode 3 (스킵) | 기존 G4a~G4d 로직 실행 (코멘트/로그만) | 항상 허용 |

- G3 완료 후 `AskUserQuestion`으로 모드 선택
- **독립 호출 모드(Linear ID 없음)**: Mode 3만 허용

### 10-2. planner agent 세션 격리

- Mode 1/2에서 `omc:planner` agent는 **격리된 세션**(Agent subagent)으로 호출
- Input: 분류 결과 전체 + plan.md 컨텍스트 + Linear payload
- 메인 컨텍스트와 격리하여 컨텍스트 오염 방지
...
| 양식 | `triage/templates/triage-plan.md` 참조 |
| 기존 plan.md | 변경 없음. triage-plan은 별도 문서 |

- 생성 후 `AskUserQuestion`으로 사용자 검토 필수 (진행/수정/거부)
- 실패 시 부분 생성 파일 삭제. 사용자 중단 시 유지 + 이어쓰기 옵션

### 10-4. Phase별 실행

- triage-plan의 Phase 순서대로 유형별 기존 라우팅 재활용
- L1/L2 → executor, L3/backlog → gen-hub, P1 → plan 수정, rework → implement, directive → feedback, limitation/reminder → Linear comment
- 동일 Phase 내 병렬 가능 항목은 병렬 실행 (Agent `run_in_background`)
- 전체 Phase 완료 후 `/커밋` (triage-plan 1개 = 1개 커밋)

> 상세 로직: [triage/references/post-pipeline.md](../skills/triage/references/post-pipeline.md)
