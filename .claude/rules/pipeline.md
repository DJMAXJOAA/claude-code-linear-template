...
| 스킬 | 설명 |
|------|------|
| `/스펙` (spec) | SDD Spec 문서 생성. 5+1 게이트 파이프라인(G1~G5 + G3a), explore + external-context → deep-interview → ralplan 스킬 체인. `docs/spec/{name}/` 단위 출력 (`overview.md` + `requirements.md` + `technical.md` + `roadmap.md`(선택)). Issue 파이프라인과 독립 |
