...
triage 분류(G0~G3) 완료 후, 3-Mode 후처리(인터뷰/자동계획/스킵)로 분류 결과를 통합 수정 계획으로 처리한다.

| 핵심 규칙 | 내용 |
|----------|------|
| 세션 격리 | Mode 1/2에서 planner agent는 격리된 Agent subagent로 호출. 메인 컨텍스트 오염 방지 |
| 커밋 원칙 | triage-plan 1개 = 1개 커밋. 전체 Phase 완료 후 `/커밋` 경유 |
| 독립 호출 | Linear ID 없는 독립 호출 시 Mode 3(스킵)만 허용 |

> 모드 선택, triage-plan 문서, Phase별 실행 상세: [triage SKILL.md §후처리 파이프라인](../skills/triage/SKILL.md), [triage/references/post-pipeline.md](../skills/triage/references/post-pipeline.md)
