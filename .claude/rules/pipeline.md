...
- **프레임워크 에이전트**: `.claude/agents/`에 정의. 스킬이 on-demand로 호출. 현재: `linear-comment-writer` (Linear comment 작성 전담)
- **공유 템플릿**: `.claude/templates/`에 정의. 에이전트/스킬이 SSOT로 참조. 현재: `comment-formats.md`, `git-doc-formats.md`
