...
> 전 type(feature, improvement, bug) 모두 Git 폴더(`docs/issue/{ID}/`) + note.md를 생성한다.
...
| Planning | `started` | pre-plan + plan (+합의 리뷰: Architect ∥ Critic 병렬→합의 병합) |
...
### 2-3a. note.md 갱신 주체 원칙

> **원칙**: 각 스킬은 note.md에서 자신이 담당하는 섹션만 갱신한다. 타 스킬 담당 섹션을 직접 수정하지 않는다. 갱신 주체는 각 스킬의 Output 섹션에서 자체 선언한다.
...
| **Done 갱신** | sub-issue가 Done이면 note.md Work Log에 해제 기록 + 기록 갱신 |
...
- 에이전트 모델 기본값: 설계=opus, 구현=sonnet, 탐색=haiku, 인과추적=sonnet (`oh-my-claudecode:tracer`, v4.8.0+)
...
파이프라인 외부 스킬이 정규 파이프라인 스킬 소유 섹션을 생성/갱신하는 경우, §2-3a 원칙에 따라 해당 스킬의 Output 섹션에서 소유권을 자체 선언해야 한다. 정규 소유자의 템플릿을 SSOT로 참조하며 복제하지 않는다. `/정리`는 note.md, prd.md를 참조하여 사후 문서화를 수행한다.
