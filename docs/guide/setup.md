...
| oh-my-claudecode (OMC) | **v4.9.3 이상** 설치 및 활성화 | [GitHub](https://github.com/yeachan-heo/oh-my-claudecode). PreToolUse/PostToolUse Hook 활성화 확인. v4.9.0 미만은 스킬 상태 충돌 발생 가능 |
| Linear 계정 | Free 플랜 이상 | [linear.app](https://linear.app) · [Linear Docs](https://linear.app/docs) |
...
2. 기본 제공되지 않는 상태(`Planning`, `In Review`)는 **Add state** 로 추가
3. 기본 제공 상태 중 불필요한 것은 삭제하거나 유지. `Todo`는 유지 필수
4. 순서를 위 표대로 드래그 정렬

> **Free 플랜 참고**: Custom Workflow State 생성이 불가한 경우, 기본 State에 매핑한다.
> - Planning → `Todo`, In Review → `In Progress`
> - 파이프라인 상태 구분이 Linear에서 완전히 반영되지 않으나, Git 문서 기반 진행에는 지장 없다.

> **마이그레이션**: 기존에 `Testing`, `Verifying` 상태가 있다면 삭제하거나 미사용 상태로 유지한다.
...
프레임워크는 Linear 공식 호스팅 MCP 서버를 사용한다. 별도 npm 패키지 설치나 API 키 발급이 필요 없다. 상세는 [Linear × Claude 연동](https://linear.app/integrations/claude) 참조.
...
│   ├── commands/                 [FRAMEWORK] (12개 커맨드)
│   └── skills/                   [FRAMEWORK] (14개 스킬)
├── docs/
│   └── guides/
...
> `docs/issue/` 디렉토리는 `/등록` 실행 시 자동 생성된다. 미리 만들 필요 없다.
...
- **Git**: `docs/issue/MYP-1/` 폴더가 존재하는지
...
Linear UI에서 Issue 상태가 `Todo` → `Planning` (또는 `In Progress`)으로 전이되는 것을 확인한다.
...
| `/점검` | 수동 테스트 결과 분류 | triage 8유형 |
| `/커밋` | Conventional Commits | |
| `/피드백` | 피드백 분류·등록 | directive/limitation/backlog |
| `/스펙` | 기능 명세 생성 | spec-driven development |
| `/조사` | 사전 조사 | investigation 스킬 |
| `/검증` | 프레임워크/구현 검증 | 무결성 또는 완전성 |
| `/병합` | 브랜치 병합 | |
| `/현황` | Linear 활성 Issue 조회 | |
| `/정리` | 사후 문서화 | 파이프라인 없이 작업 후 문서화 |
| `/릴리스` | 프레임워크 템플릿 릴리스 | |
...
| `.claude/templates/*` | 프레임워크 고정 | 수정 금지 |
...
**Q: OMC Hook 메시지가 너무 많이 출력된다**
- 환경변수 `OMC_QUIET=1`을 설정하면 Hook 메시지가 억제된다 (v4.8.1+).
- 프레임워크 스킬이 많아 Hook 메시지가 빈번할 수 있다. 필요 시 활성화한다.

**Q: OMC를 어느 버전까지 업데이트해야 하나?**
- 최소 v4.9.3 이상을 권장한다. v4.9.2에서 MCP 브릿지 크래시 버그가 있었으며 v4.9.3에서 수정되었다.
- v4.9.0 미만에서는 프로젝트 커스텀 스킬과 OMC 내장 스킬 간 상태 충돌이 발생할 수 있다.
